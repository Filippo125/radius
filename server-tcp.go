...
type tcpPacketResponseWriter struct {
	// listener that received the packet
	conn net.Conn
	addr net.Addr
}

func (r *tcpPacketResponseWriter) Write(packet *Packet) error {
...
// TCPServer listens for RADIUS requests on a packet-based protocols (e.g.
// UDP).
...
func (s *TCPServer) Serve(tcpListener net.Listener) error {
	if s.Handler == nil {
	...
	for {
		...
		if err != nil {
			s.logf("Cannot handle TCP Connection: %v", err)
			continue
		}
		...
		go func(buff []byte, remoteAddr net.Addr) {
			...
			response := tcpPacketResponseWriter{
				conn: conn,
				addr: remoteAddr,
			}
...
func (s *TCPServer) ListenAndServe() error {
	...
	pc, err := net.Listen(network, addrStr)
	if err != nil {
		return err
	}
	defer pc.Close()
